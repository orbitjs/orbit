...
test("single records found with rest should be inserted into memory and local storage", function() {
  ...
  server.respondWith('GET', '/planets/12345', function(xhr) {
      ...
      xhr.respond(200,
                  ...
                  JSON.stringify({planet: {id: 12345, name: 'Jupiter', classification: 'gas giant'}}));
  });
  stop();
...
test("records inserted into memory should be posted with rest", function() {
  ...
  server.respondWith('POST', '/planets', function(xhr) {
    var requestBody = {planet: {name: 'Jupiter', classification: 'gas giant'}};
    deepEqual(JSON.parse(xhr.requestBody), requestBody, 'POST request');
    xhr.respond(201,
                {'Content-Type': 'application/json'},
                JSON.stringify({planet: {id: 12345, name: 'Jupiter', classification: 'gas giant'}}));
  });
...
test("records updated with rest should be updated in memory", function() {
  ...
  server.respondWith('PUT', '/planets/12345', function(xhr) {
    var requestBody = {planet: {id: 12345, name: 'Earth', classification: 'terrestrial'}};
    deepEqual(JSON.parse(xhr.requestBody), requestBody, 'PUT request');
    xhr.respond(200,
                {'Content-Type': 'application/json'},
                JSON.stringify({planet: {id: 12345, name: 'Earth', classification: 'terrestrial'}}));
  });
