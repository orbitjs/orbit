...
import { OperationNotAllowed, FetchNotAllowed, UpdateNotAllowed, RecordNotFoundException, RecordAlreadyExistsException } from './lib/exceptions';
...
export default class JSONAPISource extends Source {
  ...
  _update(transform) {
    ...
    if (requests.length === 0) {
      ...
      throw new UpdateNotAllowed('JSONAPISource can only process one update request at a time.', transform);
    }
    ...
    if (!processor) {
      throw new UpdateNotAllowed('JSONAPISource can not process this update request.', request);
    }
