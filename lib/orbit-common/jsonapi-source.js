...
var JSONAPISource = Source.extend({
  ...
  _find: function(type, id) {
    ...
    } else if (id && (typeof id === 'object' && id[this.schema.remoteIdField] && !Array.isArray(id[this.schema.remoteIdField]))) {
      ...
      if (record[key]) {
        ...
        if (record.__rel) {
          ...
          if (linkDef.type === 'hasMany') {
            ...
            notFound = [];
            var query = {};
            query[_this.schema.remoteIdField] = relatedIds;


            _this.find(relatedModel, query).then(function(arrayResult) {
                if (arrayResult) {
                  arrayResult.forEach(function(relatedRecord) {
                    if (relatedRecord) {
            ...
            _this.find(relatedModel, record[key]).then(function(result) {
              if (result) {
                resolve(result);
