...
import { computeVal, isArray, isNumeric } from 'orbit/lib/objects';
import Source from './source';
import JSONAPISerializer from './jsonapi-serializer';
import { LinkNotFoundException, RecordNotFoundException, RecordAlreadyExistsException } from './lib/exceptions';
...
var JSONAPISource = Source.extend({
  ...
  init: function(schema, options) {
    ...
    if (options['buildLinkURL']) {
      this.buildLinkURL = options['buildLinkURL'];
    }
  ...
  _findLink: function(type, id, key) {
    ...
    return new Orbit.Promise(function(resolve, reject) {
      ...
      if (record.__rel && 'undefined' !== typeof record.__rel[key]) {
        var linkDef = _this.schema.models[type].links[key],
            relatedModel = linkDef.model;
        if (isNumeric(record.__rel[key])||Object.keys(record.__rel[key]).length) {
          if(linkDef.type === 'hasMany'){
            request = Object.keys(record.__rel[key]).map(function(clientid){
              return _this.schema.localToRemoteId(linkDef.model,clientid);
            });
          } else {
            request = _this.schema.localToRemoteId(linkDef.model,record.__rel[key]);
          }
          _this.ajax(_this.buildLinkURL(relatedModel, request), 'GET').then(function(raw) {
            var record = _this.deserialize(relatedModel, raw);
            if(linkDef.type === 'hasMany' && !isArray(record)){
              record = [record];
            }
            _this.settleTransforms().then(function() {
              resolve(record);
            });
          }, function(error) {
            reject(error);
        ...
        hash.beforeSend = function(xhr) {
          for (var key in headers) {
            if (headers.hasOwnProperty(key)) {
              xhr.setRequestHeader(key, computeVal(headers[key]));
            }
          }
        };
      }
...
//TODO-log        console.log('ajax error', method, jqXHR);
  ...
  buildURL: function(type, remoteId) {
    var host = computeVal(this.host),
        namespace = computeVal(this.namespace),
        url = [];
  ...
  buildLinkURL: function(type, remoteId) {
    return this.buildURL(type, remoteId);
  },
