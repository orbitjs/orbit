...
import { getUpdateRequests, UpdateRequestProcessors } from './jsonapi/update-requests';
...
import QueryOperators from './jsonapi/query-operators';
...
export default class JSONAPISource extends Source {
  ...
  _update(transform) {
    const requests = getUpdateRequests(transform);

    if (requests.length === 0) {
      return Orbit.Promise.resolve([transform]);
    } else if (requests.length > 1) {
      throw new TransformNotAllowed('JSONAPISource can only process a single request at a time.', transform);
    }

    let request = requests[0];
    let processor = UpdateRequestProcessors[request.op];

    if (!processor) {
      throw new OperationNotAllowed('JSONAPISource can not process this request.', request);
    }
  ...
  _fetch(query) {
    const url = this.queryEvaluator.evaluate(query).build();
    return this.ajax(url, 'GET').then(raw => this.deserialize(raw));
  }
