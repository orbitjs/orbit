...
var JSONAPISource = Source.extend({
  ...
  _transform: function(operation) {
    ...
    if (path.length > 2) {
      ...
      var baseURL = this.buildURL(type, resourceId);
      ...
      if (op === 'add') {
        ...
        if (op === 'replace') {
          return this.ajax(this.buildURL(type, resourceId), 'PUT', {data: this.serialize(type, data)}).then(
            function(raw) {
              // Apply the clientId directly to the payload, before it is deserialized and cached
              // TODO - reconsider this approach
              raw[_this.resourceType(type)][_this.schema.models[type].primaryKey.name] = id;
          ...
          return this.ajax(this.buildURL(type, resourceId), 'DELETE').then(function() {
            _this._transformCache(operation);
          });
        }
      }
  ...
  _findOne: function(type, resourceId) {
    var _this = this;
    return this.ajax(this.buildURL(type, resourceId), 'GET').then(
      function(raw) {
  ...
  _findMany: function(type, resourceIds) {
    var _this = this;
    return this.ajax(this.buildURL(type, resourceIds), 'GET').then(
      function(raw) {
...
//TODO-log        console.log('ajax error', method, jqXHR);
  ...
  //TODO - resourceURL
  buildURL: function(type, resourceId) {
    var host = this.host,
  ...
  // TODO- resourceLinkURL

  // TODO - linkedResourcesURL
  ...
  resourceId: function(type, id) {
    return this.serializerForType(type).resourceId(type, id);
  },

  resourceKey: function(type) {
    return this.serializerForType(type).resourceKey(type);
  },

  resourceType: function(type) {
    return this.serializerForType(type).resourceType(type);
  },
  ...
  serialize: function(type, records) {
    ...
    return serializer.serialize(type, records);
  },

  deserialize: function(type, data) {
    var serializer = this.serializerForType(type);
    var deserialized = serializer.deserialize(type, data);
