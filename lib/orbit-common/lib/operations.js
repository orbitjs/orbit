...
function operationType(operation) {
  ...
  if (path.length < 1) {
    ...
    return relationshipOperationType(operation);
...
function replaceKeyOperation(record, key, value) {
  return toOperation('replace', [record.type, record.id, 'keys', key], value);
}
...
function addToHasManyOperation(record, hasManyRelationship, value) {
  return toOperation(
    'add',
    [record.type, record.id, 'relationships', hasManyRelationship, 'data', toIdentifier(value.type, value.id)],
    true);
}

function removeFromHasManyOperation(record, hasManyRelationship, value) {
  return toOperation(
    'remove',
    [record.type, record.id, 'relationships', hasManyRelationship, 'data', toIdentifier(value.type, value.id)]);
}

function replaceHasManyOperation(record, hasManyRelationship, relatedRecords) {
  const data = {};
  relatedRecords.forEach((record) => data[toIdentifier(record.type, record.id)] = true);

  return toOperation('replace', [record.type, record.id, 'relationships', hasManyRelationship, 'data'], data);
}

function replaceHasOneOperation(record, hasOneRelationship, value) {
  const data = value && toIdentifier(value.type, value.id);

  return toOperation(
    'replace',
    [record.type, record.id, 'relationships', hasOneRelationship, 'data'],
    data);
}
...
function removeRecordFromSetOperation(record) {
  return toOperation('remove', toIdentifier(record.type, record.id));
}
