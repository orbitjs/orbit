...
export default OperationProcessor.extend({
    ...
    if (relationshipDef.inverse && !isNone(value)) {
      ...
      for (var i = 0; i < relIds.length; i++) {
        ...
        if (inverseRelationshipDef.type === 'hasMany') {
          op = addToHasManyOperation(relObject, relationshipDef.inverse, { type: type, id: id });
        } else {
          op = replaceHasOneOperation(relObject, relationshipDef.inverse, { type: type, id: id });
        }
    ...
    if (relationshipDef.inverse) {
      ...
      if (value) {
        ...
        for (var i = 0; i < relIds.length; i++) {
          ...
          if (inverseRelationshipDef.type === 'hasMany') {
            op = removeFromHasManyOperation(relObject, relationshipDef.inverse, { type: type, id: id });
          } else {
            op = replaceHasOneOperation(relObject, relationshipDef.inverse, null);
          }
  ...
  _removeDependentRecords: function(idOrIds) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (this.cache.get(dependentPath)) {
        ops.push({
          op: 'remove',
          path: dependentPath
        });
