...
export default Class.extend({
  init(opts) {
    this._initCoordinator(opts);
  },

  _initCoordinator(opts) {
    this.coordinator = opts.coordinator;
    delete opts.coordinator;
    this.coordinator = this.coordinator || new MemorySource(opts);
    expose(this, this.coordinator, 'query', 'liveQuery', 'cache', 'schema');
  },
  ...
  transform(transform) {
    return this.coordinator.transform(Transform.from(transform));
  }
});
