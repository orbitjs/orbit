...
export default class Store {
  ...
  findRecordsOfType(type) {
    return this.query(oqe('recordsOfType', type))
      .then(records => {
        if (records) {
  ...
  findRecord(type, id) {
    return this.query(oqe('record', type, id))
      .catch(e => undefined);
  }
  ...
  update(transformOrOperations) {
    const transform = Transform.from(transformOrOperations, this._transformBuilder);
  ...
  query(queryOrExpression) {
    const query = Query.from(queryOrExpression, this._queryBuilder);
