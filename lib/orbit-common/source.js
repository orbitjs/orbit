...
var Source = Class.extend({
  init: function(schema, options) {
    ...
    if (options.useCache) {
      ...
      expose(this, this._cache, 'length', 'reset', 'retrieve', 'retrieveLink', 'exists', 'hasDeleted');
    }
  ...
  /**
   ...
   @method _prepareTransformOperations
   @param {Array} [operations] An array of Orbit.Operation objects
   @returns {Array} An array of Orbit.Operation objects
   @private
   */
  _prepareTransformOperations: function(ops) {
    var result;
      ...
      coalescedOps.forEach(function(operation) {
        ...
        if (isNone(currentValue)) {
          ...
          if (eq(currentValue, operation.value)) {
            ...
            // var resolvedOps = _this._resolveConflicts(operation.path, currentValue, operation.value, operation);
            var resolvedOps = diffs(currentValue, operation.value, { basePath: operation.path });
            Array.prototype.push.apply(result, normalizeOperations(resolvedOps));
            return;
          }
        }
