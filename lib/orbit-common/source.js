...
var Source = Class.extend({
  ...
  _addLink: function(type, id, key, value, index) {
    id = this._normalizeId(type, id);
    value = this._normalizeLink(type, key, value);

    return this.transform(this._addLinkOp(type, id, key, value, index));
  },

  _removeLink: function(type, id, key, value, index) {
    id = this._normalizeId(type, id);
    value = this._normalizeLink(type, key, value);

    return this.transform(this._removeLinkOp(type, id, key, value, index));
  },

  _updateLink: function(type, id, key, value) {
    var linkDef = this.schema.models[type].links[key];
    assert('hasMany links can only be replaced when flagged as `actsAsSet` or `actsAsOrderedSet`',
           linkDef.type !== 'hasMany' || linkDef.actsAsSet || linkDef.actsAsOrderedSet);
  ...
  _addLinkOp: function(type, id, key, value, index) {
    var linkDef = this.schema.models[type].links[key];
  ...
  _removeLinkOp: function(type, id, key, value) {
    ...
    if (linkDef.type === 'hasMany') {
      if(linkDef.actsAsOrderedSet){
        var link = this.retrieveLink(type, id, key);
        var index = link.indexOf(value);
        path.push(index);
