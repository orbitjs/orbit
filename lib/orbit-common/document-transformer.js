...
export default Class.extend({
  init: function(schema, document, transformTarget, processors){
  ...
  _initProcessor: function(Processor) {
    return new Processor(this._schema, this._document);
  },  
  ...
  transform: function(operation){
    ...
    var currentValue = this._document.retrieve(path);
    var relatedOps = [];
    ...
    if (op === 'add' || op === 'replace') {
      var exists = !!this._document.retrieve(path.slice(0, path.length - 1), true);
      if (!exists) {
        return;
      }
    }
