...
var JSONAPISerializer = Serializer.extend({
  ...
  deserialize: function(type, data) {
    ...
    if (isArray(primaryData)) {
      records[type] = this.deserializeRecords(type, primaryData);
    } else {
      records[type] = this.deserializeRecord(type, primaryData);
    }
    ...
    if (linkedData) {
      ...
      Object.keys(linkedData).forEach(function(linkedResourceType) {
        ...
        records.linked[relType] = this.deserializeRecords(relType, relData);
      }, this);
    }
  ...
  deserializeRecords: function(type, data) {
    var records = [];

    data.forEach(function(recordData) {
      records.push(this.deserializeRecord(type, recordData));
    }, this);

    return records;
  },

  deserializeRecord: function(type, data) {
