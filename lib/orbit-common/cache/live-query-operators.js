    ...
    const membershipChanges = patches
      ...
      .scan((previous, operation) => {
        if (operation.value) {
          ...
          const record = this.target.get([identifier.type, identifier.id]);
  ...
  relatedRecords(context, type, recordId, relationship) {
    ...
    const membershipChanges = patches
      ...
      .map(operation => {
        ...
        const record = this.target.get(recordIdentifier.split(':'));
      ...
      operations.subscribe(
        operation => {
          ...
          if (!record) {
            ...
            const matches = cache.query(filterExpression, context);
