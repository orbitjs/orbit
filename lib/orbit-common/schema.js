 ...
 * `hasMany` - for one-to-many relationships
 ...
 `modelDefaults` simply define the field with a value of `undefined`.
...
var Schema = Class.extend({
  ...
  primaryToSecondaryKey: function(model, secondaryKeyName, primaryKeyValue) {
    var modelSchema = this.models[model];
    var secondaryKey = modelSchema.keys[secondaryKeyName];

    return secondaryKey.primaryToSecondaryKeyMap[primaryKeyValue];
  },

  secondaryToPrimaryKey: function(model, secondaryKeyName, secondaryKeyValue) {
    var modelSchema = this.models[model];
    var secondaryKey = modelSchema.keys[secondaryKeyName];

    return secondaryKey.secondaryToPrimaryKeyMap[secondaryKeyValue];
  },
          ...
          records.forEach(function(record) {
            ...
            Object.keys(modelSchema.secondaryKeys).forEach(function(secondaryKey) {
              ...
              if (altId !== undefined && altId !== null) {
                ...
                secondaryKeyDef.secondaryToPrimaryKeyMap[altId] = id;
                secondaryKeyDef.primaryToSecondaryKeyMap[id] = altId;
              }
            }, this);
          }, this);
        }
    ...
    if (modelSchema.secondaryKeys) {
      Object.keys(modelSchema.secondaryKeys).forEach(function(name) {
        ...
        if (value) {
          ...
          key.secondaryToPrimaryKeyMap[value] = id;
          key.primaryToSecondaryKeyMap[id] = value;
        }
      }, this);
    }
  },
  ...
  _mergeModelSchemas: function(base) {
    ...
    sources.forEach(function(source) {
      source = clone(source);
      extend(base.keys, source.keys);
      extend(base.attributes, source.attributes);
      extend(base.links, source.links);
    });
