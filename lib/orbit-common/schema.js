...
var Schema = Class.extend({
      ...
      for (var relationship in modelSchema.relationships) {
        if (record.relationships[relationship] === undefined) {
          record.relationships[relationship] = {
            data: defaultValue(record, modelSchema.relationships[relationship].defaultValue)
          };
        }
      }
    }
  ...
  idToKey: function(model, keyName, idValue, autoGenerate) {
  ...
  keyToId: function(model, keyName, keyValue, autoGenerate) {
    var modelSchema = this.modelDefinition(model);
    var keyDef = modelSchema.keys[keyName];
      ...
      Object.keys(data).forEach(function(type) {
        ...
        if (modelSchema && modelSchema.keys) {
          ...
          Object.keys(records).forEach(function(id) {
            ...
            if (record.keys) {
              Object.keys(modelSchema.keys).forEach(function(keyName) {
                keyValue = record.keys[keyName];

                if (!isNone(keyValue)) {
                  keyDef = modelSchema.keys[keyName];
  ...
  registerKeyMapping: function(type, id, key, value) {
  ...
  keyDefinition: function(modelName, key) {
  ...
  relationshipDefinition: function(modelName, relationship) {
    var modelDef = this.modelDefinition(modelName);
  ...
  _registerKeyMapping: function(keyDef, idValue, keyValue) {
