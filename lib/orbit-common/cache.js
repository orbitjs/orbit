...
import CacheIntegrityProcessor from './operation-processors/cache-integrity-processor';
import SchemaConsistencyProcessor from './operation-processors/schema-consistency-processor';
...
export default Class.extend(QueryProcessor, {
  ...
  init(schema, options) {
    ...
    this._initQueryEvaluator();
    this._initLiveQueryEvaluator();
  ...
  _initLiveQueryEvaluator() {
    this.liveQueryEvaluator = new QueryEvaluator(this, {
      'recordsOfType': LiveQueryOperators.recordsOfType,
      'filter': LiveQueryOperators.filter,
      'record': LiveQueryOperators.record,
      'relatedRecord': LiveQueryOperators.relatedRecord,
      'relatedRecords': LiveQueryOperators.relatedRecords,
    ...
    this.registerQueryProcessor('oql:liveQuery', (query) => {
      return this.liveQueryEvaluator.evaluate(query);
    });
  },
  ...
  rollback(transformId) {
    ...
    this._transformLog.rollback(transformId);
  },
  ...
  _rollbackTransform(transformId) {
    ...
    inverseOperations.reverse().forEach((op) => this._patch(op));
  }
});
