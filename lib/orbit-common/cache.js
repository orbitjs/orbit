...
export default Class.extend({
  ...
  transform: function(operation) {
    ...
    this._transform(this._normalizeOperation(operation), result);
    return result;
  },
  ...
  _transform: function(operation, result) {
    var _this = this;
    var op = operation.op;
    ...
    function performRelatedOps() {
      relatedOps.forEach(function(o) {
        _this._transform(o, result);
      });
      relatedOps = [];
    }

    function applyTransform(op) {
      result.completedOperations.push(op);
      result.inverseOperations = result.inverseOperations.concat(_this._doc.transform(op, true));
    }
