...
export default Class.extend(QueryProcessor, {
  ...
  transform(transform) {
    ...
    let result = new TransformResult();
    let ops = this._prepareOperations(transform.operations);

    this._applyOperations(ops, result);
  ...
  _applyOperations(ops, result) {
    ops.forEach(function(op) {
      this._applyOperation(op, result);
    }, this);
  },

  _applyOperation(operation, result) {
    var _this = this;
    var op = operation.op;
    ...
    function applyOp(op) {
      result.push(op, _this._doc.patch(op, true));
    }
