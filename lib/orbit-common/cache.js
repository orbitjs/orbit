...
/**
 ...
 @param {Boolean} [options.allowDependentOps=true] Allow schema "dependent" operations (remove cascade)
...
var Cache = Class.extend({
  init: function(schema, options) {
    ...
    this.allowDependentOps = options.allowDependentOps !== undefined ? options.allowDependentOps : true;
  ...
  transform: function(operation) {
    ...
    if (this.maintainRevLinks && (op === 'remove' || op === 'replace')) {
      this._removeRevLinks(path, normalizedOperation);
    }

    if (this.allowDependentOps) {
      dependentOperations = this._transformDependentOps(normalizedOperation);
    }
    ...
    if (this.maintainRevLinks && (op === 'add' || op === 'replace')) {
...
//    console.log('_transform', operation, track);
  ...
  _transformDependentOps: function(operation) {
    var operations = [];
