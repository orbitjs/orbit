...
var clone = function(obj) {
  ...
  if (type === "[object Date]") {
    ...
    dup = {};
    for (i = 0, list = Object.keys(obj), len = list.length; i < len; ++i) {
      key = list[i];
      val = obj[key];
...
var expose = function(destination, source) {
  var properties,
    destinationFct = function(sourceIndex) {
      var args = Array.prototype.slice.call(arguments, 1);
      return source[sourceIndex].apply(source, args);
    };
  ...
  for (var i = 0, len = properties.length, p; i < len; ++i) {
    ...
    if (typeof source[p] === 'function') {
      destination[p] = destinationFct.bind(this, p);
    } else {
...
var extend = function(destination) {
  ...
  for (var i = 0, len = sources.length, source; i < len; ++i) {
    source = sources[i];
    for (var j = 0, properties = Object.keys(source), len1 = properties.length; j < len1; ++j) {
      destination[properties[j]] = source[properties[j]];
    }
  }
  return destination;
};
...
var extendClass = function(destination) {
  ...
  for (var i = 0, len = sources.length, source; i < len; ++i) {
    source = sources[i];
    for (var j = 0, list = Object.keys(source), len1 = list.length, p; j < len1; ++j) {
      p = list[j];
      if (destination[p] &&
