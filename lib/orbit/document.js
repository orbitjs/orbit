...
var Document = Class.extend({
  ...
  retrieve: function(path, quiet) {
    return this._retrieve(splitPath(path), quiet);
  },
  ...
  test: function(path, value, quiet) {
    return eq(this._retrieve(splitPath(path), quiet), value);
  },
  ...
  _pathNotFound: function(path, quiet) {
    if (quiet) {
      ...
      throw new PathNotFoundException(joinPath(path));
    }
  },
    ...
    if (path.length > 0) {
      ...
      if (path.length > 1) {
        ...
        if (isArray(grandparent)) {
          if (parentKey === '-') {
            if (invert) {
              inverse = [{op: 'remove', path: joinPath(path)}];
            }
          ...
          if (invert) {
            if (grandparent.hasOwnProperty(parentKey)) {
              inverse = [{op: 'replace', path: joinPath(path), value: clone(grandparent[parentKey])}];
            } else {
              inverse = [{op: 'remove', path: joinPath(path)}];
            }
          }
        ...
        if (invert) {
          if (this._data.hasOwnProperty(parentKey)) {
            inverse = [{op: 'replace', path: joinPath(path), value: clone(this._data[parentKey])}];
          } else {
            inverse = [{op: 'remove', path: joinPath(path)}];
          }
        }
      ...
      if (invert) {
        inverse = [{op: 'replace', path: joinPath([]), value: clone(this._data)}];
      }
      this._data = value;
    }
    return inverse;
      ...
      if (path.length > 1) {
        ...
        if (isArray(grandparent)) {
          if (grandparent.length > 0) {
            if (parentKey === '-') {
              if (invert) {
                inverse = [{op: 'add', path: joinPath(path), value: clone(grandparent.pop())}];
              } else {
                grandparent.pop();
              }
            } else {
              ...
              if (grandparent[parentIndex] === undefined) {
                ...
                if (invert) {
                  inverse = [{op: 'add', path: joinPath(path), value: clone(grandparent.splice(parentIndex, 1)[0])}];
                } else {
          ...
          if (invert) {
            inverse = [{op: 'add', path: joinPath(path), value: clone(grandparent[parentKey])}];
          }
        ...
        if (invert) {
          inverse = [{op: 'add', path: joinPath(path), value: clone(this._data[parentKey])}];
        }
        delete this._data[parentKey];
      }
    } else {
      if (invert) {
        inverse = [{op: 'add', path: joinPath(path), value: clone(this._data)}];
      }
      this._data = {};
    }
    return inverse;
      ...
      if (path.length > 1) {
        ...
        if (isArray(grandparent)) {
          if (grandparent.length > 0) {
            if (parentKey === '-') {
              if (invert) {
                inverse = [{op: 'replace', path: joinPath(path), value: clone(grandparent[grandparent.length-1])}];
              }
              ...
              if (grandparent[parentIndex] === undefined) {
                ...
                if (invert) {
                  inverse = [{op: 'replace', path: joinPath(path), value: clone(grandparent.splice(parentIndex, 1, value)[0])}];
                } else {
