...
var Document = Class.extend({
   ...
   */
  get: function(path, quiet) {
    return this._get(splitPath(path), quiet);
  },
  ...
  test: function(path, value, quiet) {
    return eq(this._get(splitPath(path), quiet), value);
  },
  ...
  /**
   ...
   @returns {Array} Array of inverse operations if `invert === true`
   */
  patch: function(operation, invert) {
    if (operation.op === 'add') {
