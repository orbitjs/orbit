...
var Evented = {
  ...
  interface: {
    ...
    on: function(eventNames, callback, binding) {
      ...
      for (var i = 0, eventNameList = eventNames.split(/\s+/), len = eventNameList.length; i < len; ++i) {
        notifierForEvent(this, eventNameList[i], true).addListener(callback, binding);
      }
    },
    ...
    off: function(eventNames, callback, binding) {
      ...
      for (var i = 0, eventNameList = eventNames.split(/\s+/), len = eventNameList.length; i < len; ++i) {
        notifier = notifierForEvent(this, eventNameList[i]);
        if (notifier) {
          if (callback) {
            notifier.removeListener(callback, binding);
          } else {
            removeNotifierForEvent(this, eventNameList[i]);
          }
        }
      }
    },
