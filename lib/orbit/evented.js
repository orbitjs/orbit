...
var Evented = {
  ...
  interface: {
    ...
    resolve(eventNames, ...args) {
      const listeners = this.listeners(eventNames);

      function resolveEach([listener, ...listeners]) {
        if (!listener) { return; }

        return Orbit.Promise.resolve()
          .then(() => {
            const [callback, binding] = listener;
            return callback.apply(binding, args);
          })
          .then(result => result || resolveEach(listeners))
          .catch(() => resolveEach(listeners));
      }

      return resolveEach(listeners);
    },
