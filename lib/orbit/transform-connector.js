...
var TransformConnector = Class.extend({
  ...
  transform: function(transformation) {
    ...
    return this.target.transform(transformation);
  },

  /**
   @method shouldApplyTransformation
   @param {Transformation} [transformation] transformation applied to the source
   @param {TransformResult} [result] result of applying transformation to the source
  ...
  _processTransform: function(transformation, result) {
    ...
    if (this.shouldApplyTransformation) {
      if (!this.shouldApplyTransformation(transformation, result)) return;
    }

    var promise = this.transform(transformation.spawn(result.operations));
