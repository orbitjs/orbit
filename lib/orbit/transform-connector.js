...
var TransformConnector = Class.extend({
  ...
  _processTransform: function(transform, result) {
    ...
    if (this.shouldApplyTransform) {
      if (!this.shouldApplyTransform(transform, result)) return;
    }

    var promise = this.transform(transform.spawn(result.operations));
