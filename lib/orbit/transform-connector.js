...
var TransformConnector = Class.extend({
  ...
  applyTransformation: function(transformation) {
    // console.log('****', ' transform from ', this.source.id, ' to ', this.target.id, transformation);

    return this.target.applyTransformation(transformation);
  },

  /**
   @method shouldApplyTransformation
   @param {Tranformation} transformation applied to the source
   @return {Boolean} `true` if the transformation should be applied to the target
   */
  shouldApplyTransformation: null,

  /////////////////////////////////////////////////////////////////////////////
  // Internals
  /////////////////////////////////////////////////////////////////////////////

  _processTransform: function(transformation) {
    // console.log('****', ' processTransform from ', this.source.id, ' to ', this.target.id, transformation);

    if (transformation.result.operations.length === 0) return;

    if (this.shouldApplyTransformation) {
      if (!this.shouldApplyTransformation(transformation)) return;
    }

    var promise = this.applyTransformation(transformation.spawn(transformation.result.operations));
