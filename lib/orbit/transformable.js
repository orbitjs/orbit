  ...
  extend: function(object, actions) {
    if (object._transformable === undefined) {
      ...
      object.didTransform = function(operation, result) {
        ...
        if (transformation) {
          // console.log('Transformable#didTransform - matching transformation found', this.id, normalized, result);
          transformation.pushCompletedOperation(normalized, result);

        } else {
          // console.log('Transformable#didTransform - createTransformation', this.id, normalized, result);
