...
var RequestConnector = Class.extend({
  ...
  activate: function() {
    ...
    for (var i = 0, actions = this.actions, len = actions.length; i < len; ++i) {
      var handler = this.handlers[actions[i]] || this._handlerFor(actions[i]);

      this.primarySource.on(this.mode + capitalize(actions[i]),
        handler,
        this.secondarySource
      );

      this.handlers[actions[i]] = handler;
    }
  ...
  deactivate: function() {
    ...
    for (var i = 0, actions = this.actions, len = actions.length; i < len; ++i) {
      _this.primarySource.off(_this.mode + capitalize(actions[i]),
        _this.handlers[actions[i]],
